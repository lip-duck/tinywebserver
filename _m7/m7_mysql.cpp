...
void process_request(int fd, Httprequest* req, channel* ch) {
    ...
    ch->set_close_callback([fd, req, ch]() {
        ...
        g_loop->runInLoop([fd, req, ch]() {
            // ch 已经被 close_connection delete 了？
            /*if (g_loop->get_channel(fd) != ch) {
                 LOG_INFO("close_callback task: fd=%d, get_channel=%p, ch=%p", fd, (void*)g_loop->get_channel(fd), (void*)ch);
                 return;  // ch 已死，不访问
             }*/
...
int main() {
    ...
    ConnPool::instance().init("localhost", "lip", "123456", "webserver", 8);  // 【新增】
    g_pool = new Threadpool(threadpool_size);
